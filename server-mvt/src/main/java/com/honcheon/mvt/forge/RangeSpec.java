...
public record RangeSpec(
    ...
    /**
     * 돌 봉우리 — 다봉 massif 의 한 봉. <b>봉우리 부품</b> (돌산 재설계 · 사용자 피드백
     * 2026-07-16: "화산파는 봉우리 하나가 아니라 여러 봉우리의 지대 — 華山 다섯 봉").
     * 방사 몸체·능선 위에 결정론 가우시안 융기를 {@code max} 로 얹어 <b>독립 국소 최고점</b>을
     * 세운다 (정상보다 낮되 정상급). 정상(SUMMIT)만이 자연 최고봉으로 남는다 (H-2 — height &lt; lift).
     * 능선·안부가 봉우리들을 이어 하나의 바위 massif 를 이룬다. 본산 건물 단(본전 자리)은
     * 봉우리들 사이 오목한 품(안부)에 앉아 감싸인다.
     *
     * <p>결정론: 좌표의 순수 함수(가우시안·삼각함수 — 난수 0). 봉우리는 방위·반경으로 못박은
     * 절대 자리다. B-155 연속성: 가우시안은 매끄러워(C∞) z=0 급단차를 안 만든다 (검수 확인).
     *
     * @param azimuthDeg 남축(+z) 기준 방위각(도) — 봉우리 중심의 방위 (★잠정·승인 대기)
     * @param radius     주봉에서 봉우리 중심까지 방사(칸) — honsanR 안 (★잠정·승인 대기)
     * @param height     봉우리 마루 등고(base 위) — <b>lift 미만</b> 강제(정상이 최고봉) (★잠정·승인 대기)
     * @param sigma      가우시안 반치폭(칸) — 작을수록 뾰족한 봉 (★잠정·승인 대기)
     */
    public record SubPeak(double azimuthDeg, int radius, double height, double sigma) {
    }

    // ─── 돌 봉우리(다봉 massif) 잠정 파라미터 — 華山 다섯 봉 유도 · 전부 승인 대기 ──────────
    //   구성: 정상(SUMMIT, 중앙 최고봉 · lift 160) + 아래 넷 = 다섯 봉. 정확한 수는 사용자 결정.
    //   방위 유도: 곁능선 3가닥(+56·−56·+84 — Q4 확정) 위에 봉우리를 세워(곁능선 안쪽 끝을
    //     봉우리로 키움 · 사용자 피드백 옵션 그대로) 남동·남서·동 봉을 얻고, 북(180 — 남 진입축의
    //     반대, 후산 방위)에 후산봉 하나. 남축(az0)은 건물 단 사슬·등산로의 자리라 비운다 →
    //     남동·남서 봉이 그 단을 좌우에서 감싸고 정상이 뒤(북)를 막아 남쪽(내리막·조망)으로 열린 품.
    //   높이 유도: 전부 lift(160) 미만 · 146~154(정상급~조금 낮은 독립 봉). 북봉이 가장 낮다
    //     (실제 華山 北峰이 최저인 것 계승). 봉우리 마루가 능선 안부보다 15~40 솟아 봉으로 읽힌다.
    //   ★전부 잠정·승인 대기(terrain_forge_v5.md §8.2): 봉우리 수(4)·방위·반경·높이·sigma.
    private static final double SUBPEAK_SE_H = 156, SUBPEAK_SW_H = 156, SUBPEAK_E_H = 151, SUBPEAK_N_H = 146;

    /**
     * 다봉 봉우리 목록 — <b>확정값(곁능선 방위·honsanR)에서 유도</b>. 레코드 성분이 아니라
     * 파생 메서드다(레코드 서명 불변 → withTrail·호출부 무영향 · 소유 파일 밖 무수정 규약).
     * RangeField 가 이 목록을 방사 몸체에 {@code max} 로 합성한다.
     */
    public java.util.List<SubPeak> subPeaks() {
        return java.util.List.of(
                new SubPeak(56, 60, SUBPEAK_SE_H, 26),   // 남동봉 — 곁능선 +56 위, 건물 단 우측 벽
                new SubPeak(-56, 60, SUBPEAK_SW_H, 26),  // 남서봉 — 곁능선 −56 위, 건물 단 좌측 벽
                new SubPeak(84, 74, SUBPEAK_E_H, 28),    // 동봉 — 곁능선 +84 위, 지대 동쪽 확장
                new SubPeak(180, 58, SUBPEAK_N_H, 26));  // 북봉(후산봉) — 정상 뒤 후산 방위, 최저봉
    }
