...
- **닫는 조건**: ① 정본 교체 여부가 결정된다 (교체하면 story_summary 수정 + acts 재작성) ②
  분기 문법(엔딩 A~D)의 거처가 정해진다 ③ 사도천 미결(성원·결정단위)이 확정된다
...
★ㄷ 대화에서 **아직 못 받은 것** (자료 403 으로 막혀 사용자에게 요청 중):
  ① 사도천 성원 명단 (하오문·녹림·수로채·살막 중 누가) ② 칠성사 일곱의 이름·역할
  ③ 천주(창설자)가 누구인가 ④ 「연합 기술 공유」의 내용 ⑤ 외부 세력 결집(무기상·중립)이 누구인가
  ⑥ 혈교 스파이 추적의 성패가 무엇을 바꾸는가 ⑦ Stage 4 에서 갈라질 때 어느 성원이 정파 쪽으로
