...
- **닫힘**: 2026-07-15. 수리: onPanelsLoad(EntitiesLoadEvent) — 글판이 실리는 순간 명부 재구축 + 그 세계 사람들에게 가림 재적용 (멱등). 사람 눈 (사용자 재접속): 겹침 소멸 — "첫 글판과 손-한획 글판은 보입니다" = 설계 그대로 (지나온 맞이 + 현재 손, 한 번에 하나만). 부작용처럼 보인 "글판이 사라졌다"는 회귀가 아니라 **여태 조용히 고장 나 있던 순차 공개가 처음으로 작동한 것** — 세계에는 9장 전부 서 있음을 기계가 쟀다 (엔티티 계수 9 · 조성 로그 9/9)
...
### B-151 · `hidden` 키에 **독자가 없다** — 등록만 되고 아무도 안 숨긴다
- **상태**: 열림
- **분류**: 빚
- **단계**: P4
...
id 직행 허용 (표시 축만 차단 — 해금은 access/reveal 별도). jar 배포됨(21:28). ★닫힘은 인게임
확인만 남음 (비op에게 마교 총단이 지도에서 안 보인다 — 사람 눈).
...
- **분류**: 결함
- **단계**: P2
- **위치**: `server-mvt/src/main/java/com/honcheon/mvt/forge/RangeField.java`
- **의존**: —
...
- **검증**: 오프라인 렌더 재측(z=0 이웃 격차) · `RangeField` self-test 연속성 축
- **닫힘**: —

기원: 화산 오프라인 미리보기 (2026-07-15 밤, `HwasanPreview` — 서버 무접촉). PNG 높이맵에
z=0 가로 어두운 선이 보여 재봤더니 **최대 33칸 급단차** (x=−47, z=−1→0). 예: x=−80 에서
z=−1=70 → z=0=95 (+25). 원인 = 주봉 북 오프셋 8 (`RangeSpec`/`RangeField`)이 v5 산세(lift 160·
능선)와 만나 매끄러운 후산 전환이 아니라 턱이 됐다. ★"땅은 한 번만 선다 — 되돌릴 수 없는 일은
작게 시작하라"의 산 증거: **오프라인에서 잡았기에 세계를 안 깎고 고칠 수 있다.**
