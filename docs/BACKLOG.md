...
#### ★미해결 — 문 앞을 가리는 것은 소나무가 아니라 **바위 기둥**이었다 (사용자 결정 대기)
...
- **선택지**: ㉠ 문전 정면 투영(±14 × 깊이 18)을 표고까지 깎는다 — 문은 트이지만 곁 절벽이
  크게 잘린다 ㉡ 축선 쪽 한 기둥만 깎는다 ㉢ 바위는 그대로 두고 꼭대기 소나무만 없앤다
  ㉣ 지금이 맞다 (산문 곁의 암봉은 이 터의 성격이다)
- **증거 사진**: `run/stage_render/문전평면.png`(부감) · `문전동측.png`(측면 — 기둥과 그 위 소나무)
