...
★함께 잡은 결함: **입장 경주** — enter() 가 build 의 onDone 을 안 쓰고 즉시 텔레포트해, 월드
재생성 뒤 첫 입장이 잔교 없는 허공을 지나 심층 동굴(y-59)로 떨어졌다 (사용자 실측 — RCON 으로
구출). 수리 = 도착(arrive)을 onDone 뒤로 · 조성 중이면 대기 재시도. 눈: audit ferryman
