...
- **의존**: B-179 (서장 결말이 시작지를 향한다) · B-147 (3계층 헌법 — 지도가 정본) · B-182 (세가)
...
미결 (사용자 결정 대기):
① 배정 방식 — 생성 문답에서 소속을 고르는가(구파일방 제자 집안 신설?), 운명 뽑기인가.
   무소속(객잔집 등 평민 집안)은 청하현 유지인가
② 파일럿 순서 — 세가 장원 1곳(어느 성씨부터?) · 구파 산문 1곳(화산? — hwasan_brief_v5 등
   설계 문서와 지역 등록이 이미 있다) 부터 갈지
③ 지도 — 3계층 헌법상 지도가 정본(사용자 몫). 장원·산문 지도를 직접 그릴지, 시안 【제안】을
   받아 빨간펜할지
④ C안(가전 무공 입장권 — §2.8 「입장권과 빚」 문법)은 시작지와 독립이라 선행 가능 — 진행 여부
