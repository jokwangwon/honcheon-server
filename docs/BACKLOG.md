...
- **닫는 조건**: ① 순서상 빈 칸(사다리가 공중에서 시작하거나 끊기는 곳)이 채워지거나 「의도된
  ...
  requires_skill 로 첫 칸까지 닿는가) 신설 ④ 무공 0 세력의 입장권 공백 해소
- **검증**: `python3 tools/combat_audit.py --lint-only` + 사다리 눈 신설분
- **닫힘**: —
...
★채움 순서 【제안 — 하나씩·기준 겸산】: ① **모용** (자료 요청 — 0이라 최우선) → ② 소림
(장법 사다리+곤법 — 봉 기준 해소) → ③ 개방 봉법 하급 → ④ 당가 독공 사다리 → ⑤ 무당
권장법 → ⑥ 남궁 기초·경공 → ⑦ **사파·민간·군문 회차** (겸·구·중병기·부·암기·활·창 기준
해소 — 녹림·수로채·살막·군문·민간) → ⑧ 곤륜~해남 소소 → ⑨ tier 미기재 일괄 정리 →
⑩ 부채 2번째 공급원·선법 증축.
