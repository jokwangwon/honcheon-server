...
#### ★★★02~101 구간 정리·동결 (2026-08-07) — 공통 계약 한 장
사용자 확정: 「현재까지의 화산파 공통 문법을 **바닥 / 축 / 행각 / 정자 / 지붕 / 인스턴스 /
투영 비움 / 표면 소유권**으로 묶어 **이제부터 새 구역이 반드시 따라야 하는 계약**을 한 장으로」.
→ **`docs/design/hwasan/contract.md`** 신설. HANDOFF §4-표준의 **0번**으로 올렸다.
- ★그 문서는 **신고서**다 — 실물은 코드·도면이고 눈이 둘을 대조한다.
  **14항 수치 대조** + 여덟 묶음 존재 + 「새 구역을 세울 때의 순서」 존재를 눈이 잰다.
  **변이 시험**: 문서의 반폭 14→12 · 정자 5×5→7×7 로 늙히면 **2항 어긋남**으로 짖는다.
- 이 구간에서 도면화가 끝난 것: 산문 · 본전 · **행각(3구역 8자리)** · **정자(2구역 4자리)**
- 아직 통짜 코드인 것 → **전수 조사 완료**: `docs/design/hwasan/plasterhall_census.md`
  - `plasterHall` **8곳** (강당4 · 생활하5×2 · 생활중8 · 창고17 · 장로회12 · 정상암자13 · 부속암자20)
  - 옛 `pavilion()` **3곳** (정상암자13 · 전망대19 · 정원10)
  - ★**쓰임이 셋으로 갈리는데 한 부품이 다 맡고 있다**: 모임(강당·장로회) · 거처(생활·암자) ·
    저장(창고). 창 유무만 다르고 나머지가 같아 **위계도 표정도 없다** —
    도면화의 첫 일은 **유형을 가르는 것**이다.
  - ★조사는 신고가 아니라 **센 것**이다 — 눈이 소스의 호출부 수를 이 표와 대조한다
  - **다음 순서**: ~~강당(4)~~ → ~~생활(5·8)~~ → ~~창고(17)~~ → ~~장로회(12)~~ → ~~암자(13·20)~~
    **→ ★`plasterHall` 이 전부 사라졌다 (0곳). 옛 `pavilion()` 은 정원(10) 하나만 남았다**

#### ★★★D2 건축 모델링 — 강당 시험체 첫 슬라이스 (2026-08-09)
사용자 확정: 「건물 분류 작업은 닫고 **건축 모델링 강화**로. 다만 전 건물을 동시에 꾸미지 말고
**강당 한 채**를 시험체로.」 정본: `docs/design/hwasan/detail_contract.md`
- ★계율: **「예쁘게 해라」를 계약으로 만들지 않는다.** 예쁘다는 건 못 재지만
  「창이 벽보다 들어갔는가 · 기둥 아래 주초가 있는가 · 처마 밑에 서까래가 있는가」는 잰다.
- **넣은 것 넷**:
  ① **입면 깊이** — 도면에 `depth:` 절 (적주 +1 · 회벽 0 · 격자창 −1).
     방향은 도면이 안 적는다 — `outward()` 가 **자리에서** 읽고 회전도 함께 탄다
  ② **기둥을 구조체로** — 주초(석재) + 적주 + 창방 + 도리
  ③ **처마 밑 서까래** — `rafters: true` 인 지붕에만 (LOD · 근경 몫)
  ⑤ **기단** — 위계가 층수를 정한다 (auxiliary 1 · principal 2 · ceremonial 3).
     ★도면이 `foundation` 을 알아야 한다 — 모르면 **제가 앉으며 기단 윗단을 지운다**
- **2차 슬라이스 (같은 날)**: ④ 창호 · ⑥ 공포 — **여섯 다 들어갔다.**
  - ④ **모양(`family`)과 개수(`density`)를 갈랐다** — 둘을 묶으면 생활관과 창고가 다시 비슷해진다.
    창호 셋을 **그림이 아니라 규칙**으로 (`lattice` 처방을 family 가 푼다).
    **체계는 다 만들고 강당은 `W2` 만** 쓴다
  - ⑥ **자리 계약이 이름보다 먼저다**: 「공포는 적주의 머리 위에서만 시작한다」.
    ★그리고 공포는 **처마를 받친다** — 기둥 머리 *위*에 놓았더니 **지붕 위로 튀어나갔고**
    눈이 잡았다 → **지붕 바로 아래**로 내렸다. 강당은 `medium`(2단 · 밖 1 · 좌우 1) 만
  - **세 거리 고정 카메라** 신설: `D2_FAR`(-38,32,190) · `D2_MID`(-38,23,152) ·
    `D2_NEAR`(-34,22,136). **근경↑ 중경↑ 원경↓ 이면 실패** — 원경 실루엣은 깨끗하다
  - **증거**: selftest 394/394 · 도면시험 블록 `312 → 372` · 검수 깨끗(열 18002 · 유출 0)
- **증거**: selftest 387/387 · 도면시험 `블록 297 → 312` · 검수 깨끗(열 18002 · 유출 0) ·
  전·후 사진 `강당_전.png` · `강당.png`

#### ★★13·20 암자 — 은거 유형 · 그리고 옛 부품의 끝 (2026-08-08)
`config/blueprints/hwasan_hermitage.yml` 신설. 몸체 **9×7**(옛 값) · 문 **하나** · 지붕 low_gable.
- ★분류표에서 마지막까지 「미정」이던 유형. **무엇으로 갈리는가**로 정의했다 —
  같은 `low_gable` 지붕·재료를 쓰는 집이 셋이고, 갈리는 것은 **문의 수와 폭 · 크기**다:
  ```
  생활관 : 작은 문 여럿   (여러 사람)
  암자   : 문 하나 · 작다 (한 사람 · ≤9×7)
  창고   : 큰 문 하나     (물자 · 연속 ≥3)
  ```
- ★**눈이 4쌍을 되묻는다**: 암자↔거처 · 암자↔저장 · 거처↔은거 · 저장↔은거 —
  **서로의 자로 재면 전부 떨어진다.** 통과하면 계약이 아니라 같은 말을 세 번 적은 것이다
- ★**옛 정자 둘도 함께 걷었다**: 정상 암자(13)의 정자와 **절벽 전망대(19)** 를
  `hwasan_pavilion.yml` 인스턴스로. 전망대는 예상대로 **인스턴스 하나로 끝났다** —
  정자 도면이 이제 **네 구역**(외원 2 · 중정 2 · 정상 1 · 전망대 1)에 앉는다
- **증거**: selftest 380/380 · 도면시험 암자 `칸 126 · 지붕 2` · 자리·기단 대조 2/2 ·
  검수 깨끗(열 18002 · 유출 0) · 사진 `정상암자.png` ·
  ★**`plasterHall` 0곳 · 옛 `pavilion()` 1곳** (눈이 소스를 직접 센다)

#### ★★12 장로회 — 모임 계열의 얕은 변형 (2026-08-08)
`config/blueprints/hwasan_council.yml` 신설. `family: assembly` · `usage: council` ·
몸체 **15×7** · 처마 **1** (강당 17×11 · 처마 2) · 지붕 **sweep**(계열이 같으니 지붕이 같다).
- ★사용자 확정대로 **강당을 찌그러뜨리지 않았다** — 「같은 계열의 **다른 평면 변형**」.
  그리고 숫자를 미리 정하지 않고 **자유 영역부터 쟀다**.
- ★**실측이 배치를 정했다**: 중앙 계단 봉투 `x18~26` 이 **남북을 잇는다** (북 z0~11 · 남 z23).
  한 채가 축 위에 앉으면 **정상 암자로 가는 길이 끊긴다** →
  **두 채가 축을 사이에 두고 마주 본다** (장로회는 마주 앉는 자리다).
- ★**처마를 2 → 1 로 줄인 것도 실측이 정했다**: 팔작은 반폭+내밈만큼 퍼져서, 처마 2 면
  지붕이 축의 틈을 9 → 7 로 좁힌다. 눈이 **틈 9 ≥ 계단 봉투 9** 를 잰다
- ★**눈의 표본을 옮겼다**: 「전고 상자 ⊃ 지상 상자」를 장로회로 재고 있었는데 이제 기단만이라
  두 상자가 같다 → 아직 코드가 지붕을 얹는 **망루(11)** 로 표본을 옮겼다.
  눈을 지운 게 아니라 **재는 자리를 옮긴 것**이다
- **증거**: selftest 371/371 · 도면시험 `칸 210 · 블록 450 · 지붕 2` ·
  자리·기단 대조 2/2 · 틈 9 · 검수 깨끗(열 18002 · 유출 0) ·
  조사 표 3곳 → **2곳** · 사진 `장로회.png` · `장로회부감.png`

#### ★★17 창고 — 저장 유형 (2026-08-08)
`config/blueprints/hwasan_storage.yml` 신설. 몸체 **15×11**(옛 값 그대로) · 자리 [10,5].
- ★사용자가 짚은 함정을 피했다: 「생활용 하나 만듦 → 창고도 비슷하니 그대로 사용 →
  다시 하나가 여러 쓰임을 먹는다」. **지붕·재료는 공유(low_gable)하되 평면 계약은 갈랐다.**
  ```
  생활 : 작은 문 여럿(연속 ≤2) · 창 많음 · 같은 칸 3연속 금지
  저장 : 큰 문(연속 ≥3)        · 창 ≤20%   · 벽 ≥50%
  ```
- ★**눈이 그 차이를 되묻는다**: 「창고를 **거처의 자**로 재면 떨어지는가」 ·
  「생활관을 **저장의 자**로 재면 떨어지는가」 — **둘 다 떨어진다.**
  통과해 버리면 계약이 아니라 **같은 말을 두 번 적은 것**이다.
- ★**자를 두 번 고쳤다** (문턱을 낮춘 게 아니라 자가 틀렸다):
  ① 벽·창 비율을 **정면에서만** 쟀더니 큰 문이 벽을 먹어 0.40 → **둘레 전체**로 잰다
  ② 「같은 칸 5연속 금지」가 **다섯 칸 큰 문**을 잡았다 → **실한 칸만** 센다
     (그 다섯 칸은 이 집의 **정의**다)
- **증거**: selftest 362/362 · 도면시험 `칸 165 · 블록 263 · 지붕 1` ·
  자리·기단 대조 · 계단 봉투 0 · 유출 0 · 옛 `plasterHall` 0 ·
  검수 깨끗(열 18002 · 유출 0) · 조사 표 4곳 → **3곳** · 사진 `창고.png`

#### ★★05·08 생활관 — 거처 유형 (2026-08-08)
`config/blueprints/hwasan_residence.yml` 신설. **한 도면이 세 자리**(생활 하 2 · 생활 중 1).
- `rank: auxiliary` · `usage: residence` · 몸체 **13×7** · 벽 **4**(강당 5보다 낮다) ·
  지붕 **low_gable** · 정면에 **작은 문 셋**
- ★**강당의 축소판이 아니다** (사용자): 동선이 다르다 —
  강당 「밖 → 중앙 문 → 큰 내부」 / 생활관 「밖 → 작은 문 여럿 → 반복되는 생활 단위」.
  그래서 「축선 통행 개구」를 **안 잰다**. 대신 **출입구 ≥2 · 연속 개구 ≤2(대문 금지) ·
  같은 칸 3연속 금지(리듬) · low_gable** 을 잰다.
- ★**지붕도 갈린다**: 생활관까지 `sweep` 를 쓰면 대부분이 핵심 전각처럼 보여 **위계가 죽는다**
- ★**창고와 지붕·재료는 공유해도 `usage` 계약은 공유하지 않는다** — 그러지 않으면
  하나가 다시 여러 쓰임을 먹는다 (`plasterHall` 이 그랬다)
- **실측이 크기를 정했다**: 두 패드(36×32 · 32×28) 다 거의 비었고 **서쪽 어귀만** 막혔다
  (5: x0~2 z14~18 · 8: x0~2 z12~16) → 13×7 + 처마 1 = 15×9 가 세 자리에 다 든다.
  여백이 남는다고 키우지 않았고 줄이지도 않았다
- **눈**: 도면 자기 계약 · 위계·쓰임 · 낮은 맞배 · 벽이 강당보다 낮다 · 자리 대조 3/3 ·
  계단 봉투 0 · 유출 0 · 옛 `plasterHall` 0 ·
  ★**「생활관을 모임의 자로 재면 떨어진다」** (계약이 정말 다른지 되묻는 눈)
- **증거**: selftest 352/352 · 도면시험 `칸 273 · 블록 504 · 지붕 3` ·
  검수 깨끗(열 18002 · 유출 0) · 조사 표 7곳 → **4곳** · 사진 `생활관.png` · `생활부감.png`

#### ★★04 강당 — 모임 유형 1호 (2026-08-07)
`config/blueprints/hwasan_hall.yml` 신설. **유형을 가른 첫 도면**이다.
- `rank: principal` · `usage: assembly` · 몸체 **17×11** · 단층 벽 5 · 지붕 **sweep**(팔작) ·
  정면 가운데 **3칸 통행 개구**
- ★모임 유형은 **지나 들어가는 집**이라 정문급 계약(축선 통행 개구)을 지킨다.
  행각·정자(auxiliary)와 다르고, 산문·본전과 같은 자를 쓰되 **단층**이라 위계는 크기·지붕이 낸다.
- ★**몸체를 안 줄였다** — 패드 40×30 이 거의 비어 있어 21×15 발자국이 가운데 [11,9] 에 든다
  (계약: 축소는 마지막 선택)
- ★**장로회(12)는 이 도면을 못 쓴다**: 패드 44×24 를 중앙 계단이 세로로 관통해 자유 띠가
  z12~22(11칸)뿐이라 깊이 15 가 안 들어간다 → 그 회차에서 **얕은 변형**을 정한다
- **눈**: 도면 자기 계약 · 위계·쓰임 · 몸체 17×11 · 팔작 단층 · 자리·기단 대조 ·
  계단 봉투 0 · 유출 0 · **강당이 옛 `plasterHall` 을 안 부른다**
- **증거**: selftest 342/342 · 도면시험 `칸 187 · 블록 297 · 지붕 1` ·
  검수 깨끗(열 18002 · 유출 0) · 조사 표가 8곳 → **7곳**으로 함께 줄었다 ·
  사진 `강당.png` · `강당사시.png`

#### ★★02 외원 — **최종 상태** (2026-08-07 · 승인)
장부가 중간 상태를 최종값처럼 들고 있어 사용자가 잡았다. **최종은 이것 하나다.**
```
행각 배치   전   : 4토막 (남·북 · 옛 퍼걸러 문법)
            중간 : 2토막 (북측 좌우 — E-02 가 정자에 모서리를 내주며)
            후   : ★삼면 네 모듈 (북서·북동·서·동) · 남측 개방
외원 부품   정자 2 + 행각 4 + 등롱열 2 = ★8
            ※ 「부품」은 parts() 가 낳는 발자국 단위다. 행각 넷은 한 도면
              (hwasan_outer_corridor.yml)의 네 인스턴스이므로, **모듈 인스턴스**로 세면
              도면 1 · 인스턴스 4 다. 두 계수는 세는 자가 다르다 — 섞어 적지 않는다.
도면시험    칸 120 (6×5 모듈 × 자리 4) · 지붕 4
```
- **★★02 외원 구조 동결** (사용자 판정 2026-08-07):
  E-01 공공 포장 · E-02 의례축 11 · E-03 삼면 행각 · E-08 instances 도면 ·
  E-07 정자/hip_pyramid — **전부 승인·동결**.
  `행각 도면 1 / 인스턴스 4 · 정자 도면 1 / 인스턴스 2 · selftest 313/313 · 유출 0`
- **여기서 더 장식하지 않는다** — 위쪽으로 문법을 전파하는 것이 다음이다. 사용자 근거:
  「산문 → 열린 외원 → 삼면 회랑 → 정자까지 언어가 완성됐는데 바로 위 종문·중정에 옛
  퍼걸러가 남아 있으면, 플레이어가 올라가며 **새 언어에서 옛 언어로 퇴행**하는 구간이 생긴다」
- **다음 순서 (사용자 확정)**: ① **E-06 종문·중정 행각** — 외원에서 확정한 hwasan_common /
  low_gable 재사용, 다만 **중정의 위계에 맞게 길이·폐쇄율만 새로 결정** ②  E-04 연무장 모래
  ③ 11 망루 사모 전환 (검증된 hip_pyramid 를 tower profile 로)
- ★**E-04 의 주의 (사용자)**: 「외원이 석전 100% 가 됐다는 이유로 **연무장도 돌바닥으로 만들면
  안 된다**. `sandField` 가 의도된 수련 재료인지, 사암 해시와 섞여 잡티가 된 것인지를 다시
  **블록으로 재는** 회차가 되어야 한다」 — 모래를 없애는 것부터 전제하지 않는다

#### ★02 외원 — 첫 진단 (2026-08-06)
회귀 카메라 셋을 새로 동결했다: 부감 `(-2,78,146) yaw180 pitch90` · 측면 `(30,4,146) yaw90 pitch-5` ·
정면 `(-2,3,170) yaw180 pitch-4`.
- `지움` **E-01 · 마당이 누런 바둑판** — **실측** 석전 63% : 매끈사암 37%.
  진범은 `HwasanCampusBuilder.sandyRepave` — 구역 1·2·6·13·101 바닥에 해시로 사암을 30%+10%
  덧칠하고 있었다. `paveMaterial` 은 2026-08-05 에 이미 다섯→하나로 고쳤는데(D-43) **그 위에
  도로 칠하는 손**이 있었다 — 문전 조경 소나무와 **같은 병**.
  → `publicRepave` + `publicPaveMaterial()` 한 곳으로. 재측정 **100% 석전**.
  ★가르는 자: **위치가 정하면 구조, 해시가 정하면 잡티다.**
- `지움` **E-02 · 중앙축이 막혔다** — 정자 두 채가 `cx±7` 에 서서 산문→종문 축을 나눠 먹었다.
  사용자 확정: **정자 유지 · cx±7 폐기 · 좌우 외곽으로 · 중앙 11칸 의례축 확보 ·
  산문 ±14 상자를 외원에 복제하지 않는다** (「근거리 투영 계약이라 전체에 늘리면 광장이
  긴 활주로처럼 보인다」).
  → `outer_court_axis` 계약 신설: 폭 11 · 산문↔종문 · **교차로 잰다**(좌표가 아니라).
  ★자리를 두 번 고쳤고 **눈이 둘 다 잡았다**: ① 패드 32 는 짝수라 동쪽 반이 한 칸 좁아
  `cx+7·half3` 정자가 축 경계에 물렸다 → ±11·half2 ② 남측 행각을 물려 모서리를 비우려니
  그 토막이 **길이 0 이 되어 조용히 사라졌다**(원래 6행) → 남측 행각을 뺀다.
  이때 외원 부품이 8 → 6 이 됐다 (정자2+행각2+등롱열2). ★**이것은 중간 상태다** —
  E-03 이 삼면 네 모듈로 다시 세워 **최종은 8** 이다 (아래 「최종 상태」).
- `지움` **E-05 · surface_ownership** — 「한 표면에는 최종 재료 소유자가 하나만 존재한다」
  (사용자). 소스를 읽어 `sandyRepave(` 가 되살아났는지 보는 눈을 세웠다.
- `지움` **E-03 · 행각에 벽이 없다** — 옛 행각은 기둥 + 검은 지붕뿐이라 **퍼걸러**로 읽혔다.
  사용자 확정: 회랑은 재료만이 아니라 ① **마당 경계를 연속으로** ② **중앙축과 별개의 측면
  동선**으로 성립한다. → **북·서·동 삼면 · 남측 개방 · ㄷ자 한 채로 합치지 않고 네 모듈 ·
  모서리 열린 전이칸 · 정자와 이격**.
  문법: 마당 쪽 열림 / 외곽 쪽 회벽+격자창 · 반복 단위 **적주|회벽|격자창|회벽**(주기 4) ·
  산문과 **공유**(적주·회벽·격자·어두운 지붕·석재 기단) · **낮춤**(단층 4 · 낮은 맞배 ·
  얕은 처마 · 기단 1 · 현판 없음).
  ★**실측이 자리를 세 번 고쳤고 눈이 전부 잡았다**: ① 동측 처마가 패드 밖(계율 #4)
  ② 서·동 행각이 **측면 계단 어귀(로컬 z16 · 반폭 1)** 를 막았다 → 어귀 위쪽 6칸으로
  ③ 등롱 열 옛 자리(cx±13)가 **새 행각 안**이었다 → 축과 행각 사이 빈 띠(로컬 x8·24)로.
  눈의 자도 두 번 고쳤다 — 정자/행각을 「긴 쪽」이 아니라 **얇은 쪽(5~7)** 으로 가르고,
  측면 모듈은 어귀를 피하느라 폭보다 짧아 **방향이 아니라 붙은 가장자리**로 판정한다.
- `진행` **E-06 · 종문(6)·중정(101) 행각** — 외원 문법을 위로 전파했다 (사용자 순서 확정).
  ★도면 기계를 넓혔다: `instances` 의 자리마다 **`pad`** 를 줄 수 있다 →
  **한 도면(`hwasan_outer_corridor.yml`)이 세 구역에 앉는다** (외원 4 · 종문 2 · 중정 2 = 8).
  안 적으면 종전대로 `meta.origin_pad` 라 옛 도면은 그대로 선다.
  **옛 `cloister`(퍼걸러)는 코드에서 걷어냈다** — 이제 아무도 안 쓴다.
  - ★**측면 모듈은 아직 안 세웠다 (E-06b)**: 측면 계단 어귀가 구역마다 다르게 들어온다 —
    **실측** 외원 로컬 z16 · 종문 z15 · 중정 z14 (전부 반폭 1). 외원은 어귀 위 6칸이 딱
    맞았지만 종문·중정은 창이 좁고 중정은 정자(cz+7)와도 겹친다. **추측으로 밀어 넣지 않는다** —
    외원이 그랬듯 제 실측 회차에서 자리를 정한다
  - ★중정 북동을 [22,1] → **[24,1]** 로 옮겼다: 계단 9→101 어귀(로컬 x18 반폭 3)에 걸렸다 (눈이 잡았다)
  - 종문 부품 5 → **3** (문 + 행각 2). 그 눈을 고친 것이 이 결정의 기록이다
  - **증거**: selftest 313/313 · 도면시험 `칸 240 · 지붕 8` (6×5 모듈 × 자리 8) ·
    **도면의 자리와 코드의 기단이 칸까지 같다 (세 구역 전부 8/8)** · 검수 깨끗(열 18002 · 유출 0)
- `지움` **E-06b · 종문·중정의 측면 행각 — 창을 다 쟀고, 안 세우기로 했다**
  ★**재는 자를 눈과 맞췄다**: 처음 `lane.half` 로 재서 한 칸씩 어긋났는데, 검수는
  **`lane.rail`(=half+1)** 에 길이 앞뒤 한 칸을 더한 봉투를 쓴다.
  ```
  종문 서 : z13~17 막힘 → 자유 z0~12      동 : 막힘 없음
  중정 서 : z12~16 막힘 → 자유 z0~11      동 : 막힘 없음
  ```
  북측 모듈이 처마까지 z0~6 을 쓰므로 서측에 남는 창은 **종문 6칸(7~12) · 중정 5칸(7~11)** 인데,
  **전이칸(≥1)을 두면 6칸 모듈이 안 들어간다.**
  동쪽만 세우면 좌우가 깨지므로 **대칭을 깨느니 기다린다** (산문 「정면 좌우 대칭」 동결과 같은 자).
  **★사용자 확정 (2026-08-07): ㉢ — 종문·중정은 북측 한 쌍으로 둔다.**
  삼면 회랑은 **외원의 표식**으로 남는다: 외원은 「머무는 마당」이라 삼면이 어울리지만
  종문은 **지나가는 문**이고 중정은 더 작다. 억지로 채우지 않는 것이 곧 **위계의 표현**이다.
  → 이 결정은 이미 눈에 잠겨 있다: 「한 도면이 세 구역에 앉는다 — 외원 4 · 종문 2 · 중정 2」.
  숫자가 바뀌면 그 눈이 짖고, **그 눈을 고치는 것이 곧 다음 결정의 기록**이 된다.
- `지움` **E-07b · 중정(101) 정자** — 외원과 **같은 도면**을 쓴다 (2026-08-07).
  ★사용자 원칙을 그대로 적용했다: **「도면은 재사용하지만 배치는 공간이 결정한다」** —
  「외원에 둘이니 중정도 둘」이라는 **역방향 설계를 하지 않았다**.
  **중정 패드(30×28)를 통째로 훑어** 자유 영역부터 냈다 (계단 봉투는 검수와 같은 자로):
  ```
  중앙 계단 봉투 x14~22 (z0~16)   남측 계단 봉투 x11~19 (z27)
  서측 계단 봉투 x0~2 (z12~16)    북측 행각 x2~7 · x24~29 (z0~6)
  → 남는 곳은 남쪽 띠 z17~26 뿐. 거기서 9×9(몸체 5 + 처마 2) 둘이
    중앙 통로(x9~20)를 열어 둔 채 들어간다 — <b>수가 아니라 자리가 정했다</b>
  ```
  몸체·지붕을 줄이지 않고 **외원의 5×5 + 처마 2 를 그대로 재사용**했다 (자리를 먼저 옮겼다).
  `hwasan_central_pavilion.yml` 같은 **복제본은 만들지 않았다** — `pad: 101` 인스턴스 둘.
  - **눈**: 정자 실재 · 축 교차 0 · **계단 봉투 교차 0** · 패드 유출 0 · `hip_pyramid` ·
    `open_sides ≥ 3` · **중정이 옛 `pavilion()` 을 안 부른다**
  - ★**눈의 자를 하나 되돌렸다**: 처음 계단 봉투를 **처마까지** 재서 외원 정자가 1칸 걸렸는데,
    이 저장소는 통로 겹침을 **지상 발자국**으로 본다 (`groundBoxes` 의 「처마 제외」).
    머리 위 처마는 걷는 것을 막지 않는다 — **자가 과했다**. 유출은 반대로 처마까지 본다
    (그건 블록이 실제로 패드 밖으로 나간다)
  - **증거**: selftest 325/325 · 도면시험 `칸 100 · 지붕 4` (5×5 × 자리 4) ·
    자리와 기단 대조 4/4 · 검수 깨끗(열 18002 · 유출 0) · 사진 `중정부감.png` · `중정정자.png`
- `지움` **E-09 · 11 망루 사모 전환** (2026-08-07) — E-07 이 만든 `hip_pyramid` 를
  **tower 비례**로 실물에 적용했다 (처마 1 · 오름 5 · 캡 2). 정자(처마 2 · 오름 3 · 캡 1)와
  **같은 계열이되 실루엣이 다르다** — 「같은 타입이되 같은 지붕을 복사하지 않는다」가 실물이 됐다.
  ★**꼭지는 발자국이 정한다**: 7칸 상자는 rise 를 올려도 네 켜에서 닫힌다. 그걸 모르면
  **캡이 한 칸 떠** 공중에 박힌다 → `top = min(rise, 짧은변/2 + 1)` 로 잠갔다.
  **실측 대조** — 켜별 덮인 칸:
  ```
  정자 (처마2·오름3) : 56 → 20 → 8 → 1 → 1        (4켜)
  망루 (처마1·오름5) : 64 → 44 → 32 → 16 → 8 → 1 → 1 → 1   (7켜)
  ```
  **증거**: selftest 319/319 (눈 3건 — 조성부를 찾았는가 · 사모인가 · 급한가) ·
  검수 깨끗(열 18002 · 유출 0) · 사진 `run/stage_render/망루.png` · `망루원경.png`
- `지움` **E-07 · 정자 문법** — 정자도 도면이 갖는다 (`config/blueprints/hwasan_pavilion.yml`).
  **사모지붕(`hip_pyramid`)을 신설**해 화산파 지붕 문법이 셋이 됐다 (사용자 확정 2026-08-07):
  `low_gable`(행각·생활 부속) · **`hip_pyramid`(정자·망루)** · `sweep`(핵심 전각).
  이 분류가 **건물 역할과 실루엣을 동시에** 가른다.
  - **같은 타입, 다른 비례**: 정자 `profile pavilion · rise 3 · eave 2`(낮고 넓다) ·
    망루 `profile tower · rise 5 · eave 1`(높고 급하다). ★**망루 실물 전환은 11 망루 회차**
    (한 커밋에서 둘을 다 바꾸면 회귀 범위가 넓어진다 — 사용자)
  - **부속급 안에서도 계약이 갈린다**: `meta.usage` — 행각은 「마당 열림·외곽 닫힘」,
    정자는 **「사방 개방(면의 절반 넘게 · 3면 이상)」 · 정사각 · 사모**를 지킨다
  - **사모 1호는 정사각 전용** — 자유형 다각형 지붕 엔진은 만들지 않는다
  - ★**실측이 세 번 고쳤다**: ① 9×9 + 처마 2 = 13칸이 **패드 밖**으로 나갔다 → 몸체 5×5
    (동쪽은 축과 패드 끝 사이가 10칸뿐) ② 변 가운데 기둥까지 두니 **열린 면 0** 이라
    계약이 깨졌다 → **모서리 적주 넷만** ③ 켜마다 테두리 한 줄만 놓아 처마와 몸체 사이가
    **뚫렸다**(y3 의 x-15~-11 전부 air) → **고리띠**로 채운다
  - ★**한 면에 한 재료**로 되돌렸다: E-03·E-07 이 심층암 타일과 조약 심층암을 섞고 있었다.
    지붕은 `roofCube`/`putRoofStair` 의 **조약 심층암** 한 계열이고, **신고표에서도 타일을 걷었다**
    (신고가 실물보다 넓으면 눈이 헛것을 지킨다)
  - ★**눈의 자를 좁혔다**: D-20(산문 시야 회랑 ±13)이 **외원 패드까지** 재고 있었다.
    외원은 산문 너머·위라 그 시선에 안 들고, E-02 로 **제 축 계약**을 갖는다.
    ※옛 정자(9칸)는 상자가 회랑 밖으로 삐져나가 **우연히** 통과하고 있었다 — 그 눈은 외원에
    대해 실제로는 아무것도 안 지키고 있었다
  - **증거**: selftest 313/313 · 도면 자기 계약 통과 · 자리와 기단 대조 2/2 ·
    **RCON 실측** 켜별 덮인 칸 **56 → 20 → 8 → 1 → 1(캡)** (모든 칸이 어느 켜에선가 덮인다) ·
    실기동 `칸 50 · 블록 64 · 지붕 2` · 사진 `정자근경.png` · `정자사시.png` · `외원부감.png`
  - **재현**: `혼천 도면시험 hwasan_pavilion hwasan`
- `지움` **E-08 · 행각 도면 정본(yml)** — `config/blueprints/hwasan_outer_corridor.yml` 신설.
  **한 도면이 네 모듈을 낳는다** (사용자 확정 2026-08-07).
  - 도면 기계에 **자리·방향**을 넣었다: `instances: [{id, at:[col,row], rotate}]`.
    안 적은 도면(산문·본전)은 **종전대로 패드 한가운데 한 장** — 회귀 없음.
    회전은 평면·지붕 상자·**살창 법선**까지 함께 돈다 (법선이 안 따라가면 창이 뒤집힌다).
  - **위계**를 도면이 선언한다: `meta.rank: auxiliary`. 계약이 갈린다 — 정문급은
    「축선에 통행 개구」를, **부속급은 「마당 쪽이 열렸는가·외곽 쪽이 닫혔는가」** 를 지킨다.
    `validate()` 가 **도면에서** 잰다: 마당 0.67 ≥ 0.60 · 외곽 1.00 ≥ 0.55 ·
    같은 칸 3연속 금지 · 지붕은 `low_gable` 이어야 한다 (팔작은 위계를 흐린다).
  - **낮은 맞배**를 지붕 문법에 추가 (`type: low_gable` → `gableRoof`). 정문급 `sweepRoof`
    (귀솟음 팔작)와 가른다. 상자를 그대로 받는다 — 중심+반폭은 짝수 폭에서 한 칸 모자란다.
  - **역할을 갈랐다**: 캠퍼스는 **기단(지면 일)** 만 포장면에 깔고, 몸체·지붕은 **도면**이 갖는다.
    BlueprintBuilder 는 포장면 **위**만 비우므로 기단이 살아남는다 — 형태가 두 곳에 안 적힌다.
  - **증거**: selftest 300/300 · 도면 자기 계약 통과 · **도면의 자리와 코드의 기단이 칸까지
    같다**(4/4) · 실기동 `칸 120 · 블록 176 · 지붕 4` · **RCON 실측**: 살창 `open=true`(서 있다) ·
    인방이 위아래를 끊는다 · 적주 `stripped_mangrove_log` · 회벽 `bone_block` ·
    마당 면 `air`(열린다) · 사진 `외원정면.png` · `행각서측.png` · `행각북서.png`
  - **재현**: `혼천 도면시험 hwasan_outer_corridor hwasan`
- `지움` **E-04 · 연무장(3·14) 모래 마당** — **모래는 남기고 얼룩만 걷었다** (2026-08-07).
  ★사용자 주의를 먼저 지켰다: 「외원이 석전 100% 가 됐다는 이유로 **연무장도 돌바닥으로
  만들면 안 된다**」 — 모래를 없애는 것부터 전제하지 않고 **블록으로 먼저 쟀다**.
  **실측** (연무장 하 · 표본 112칸): 모래 80% : 매끈사암 **20%**.
  그 20% 는 `floorMod(x*7 + z*13, 5)` 가 뿌린 것 — 외원을 누렇게 만들던 `sandyRepave` 와
  **같은 꼴의 해시**다. 가르는 자는 그대로: **위치가 정하면 구조, 해시가 정하면 잡티.**
  → 판정이 둘로 갈렸다:
  - **모래는 남는다** — 연무장의 모래는 조닝이 아니라 **쓰임**이다 (밟고 구르는 바닥).
    조닝은 **표면 자체가 다른 재료**인 것으로 드러나야 하고, 다른 재료를 위에 **덧칠하는**
    것이 틀린 표현이다.
  - **얼룩은 걷는다** — 20% 사암은 구조가 아니라 잡티였다.
  - **눈**: 「연무장 바닥이 한 재료다」 + 「**수련 바닥이 공공 마당과 다른 재료다**」
    (둘이 같아지면 연무장이 광장이 된다) + 「모래를 없애지 않았다」
  - **증거**: selftest 316/316 · **재측정 모래 100%** (같은 자·같은 표본 112칸) ·
    검수 깨끗(열 18002 · 유출 0) · 사진 `run/stage_render/연무장.png`
