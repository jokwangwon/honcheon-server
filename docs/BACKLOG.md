...
★사용자 문서 등재 둘 (2026-07-15): `docs/design/hwasan_campus_architecture.md` (건축 보강 —
시설 16종·수직 성장·팔레트) + `docs/design/hwasan_domain_design.md` (**영역 설계** — 11구역:
외곽 평원→정상 · 자연 70/건축 20/인공 10 · 구역별 밀도/식생/NPC 표 · 랜드마크 · 생성 순위
8단계). 둘 다 사용자 직접 작성 · 우선순위 계약 내장 (사용자 확정 > 등록부 > 헌장 > 보강 문서).
영역 문서로 **B-147 ①(영향권 브리프)의 화산 몫이 정본으로 채워졌고**, 사다리 앵커 R_c 는
캠퍼스가 아니라 영역 전체를 담는 반경으로 다시 잰다 (브리프 초안에 반영 지시됨).
