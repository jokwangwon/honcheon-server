...
- **검증**: 세가 캐릭터로 `/혼천 대화` — 첫 만남에 가문 이름을 대면 위반
- **닫힘**: —
...
### B-111 · **PvP 쓰러짐·의학당 회수가 배선되지 않았다** (경험 정본 §2)
- **상태**: 열림
- **분류**: 미완
- **단계**: P1
- **위치**: `docs/design/death_and_legacy.md`
- **의존**: —
- **닫는 조건**: 최종장 전 PvP 는 영구 사망 없음 — 도주·쓰러짐 판정 → 의학당(의방) 회수가 실제로 돈다 (기존 죽음 파이프의 다운→이송 문법 재사용)
- **검증**: `docs/design/death_and_legacy.md` · PvP 실기동 (쓰러짐→의방 회수)
- **닫힘**: —

사용자 확정 (2026-07-14, D-0 Q2): 살상 PvP 는 메인스토리 최종장의 것이다.
※ PvE 죽음의 영구성은 기존 규칙 유지 (사용자 재확인 대기 — 기본값 유지).
