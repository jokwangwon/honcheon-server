...
ROOF_MID = 6.6          # 계단 중앙값 — 너울·얼룩이 여기서 위아래로 흔들린다
# 세로 너울 (x % 8) — '마루'가 아니라 **완만한 결**이다. 하이라이트도 골도 없다: 한 주기에 걸쳐
#   부드럽게 밝아졌다 어두워질 뿐 (진폭 ±2.1단 ≈ 명암 폭 16). 블록당 둘 — 저주파의 하한이다.
#   지난 판의 ROOF_CURVE는 폭이 7단(≈70)이었고 1px 만에 3.6→0.8로 꺾였다. 그 급락이 리벳이었다.
# ★ 표는 **순환 연속**이라야 한다 — 마지막 값과 첫 값의 낙차가 내부 낙차보다 크면 그 자리가
#   좌우 랩 이음매가 된다 (실측: 비순환 표로 이음매 4.24. 표 끝에서 3.1단이 한 번에 꺾였다).
#   여기선 인접 낙차 최대 1.8, 랩 낙차 1.6 — 랩이 여느 경계와 구별되지 않는다.
ROOF_SWELL = [2.0, 1.9, 0.9, -0.5, -1.7, -2.1, -1.4, 0.4]
# 단(段) 이음 — 한 장에 **한 줄만** (주기 16). 지난 판은 주기 8이라 블록마다 둘이었고,
#   그게 세로 결과 교차해 격자를 만들었다. 이제 한 줄 — 얕은 그늘 + 바로 아래 옅은 턱.
#   깊이 -3.0단 ≈ 명암차 11 (지시된 10~15 대역). 이보다 짙으면 다시 '자로 그은 선'이 된다.
# ★ 줄은 **y=8(장 한가운데)**에 둔다. y=0에 두면 그 줄이 곧 상하 랩 경계가 되어, 텍스처에서
#   유일하게 강한 가로 경계가 하필 이음매 자리에 앉는다 (실측: 이음매 4.65 — 린트가 옳다.
#   랩 경계가 이 장의 다른 어떤 가로 경계보다 세다는 뜻이니 실제로 눈에 띄는 띠가 된다).
#   한가운데로 옮기면 랩 경계는 매끈한 기와 몸통끼리 만나 완전히 이어진다.
ROOF_COURSE = {8: -3.0, 9: +0.7, 10: +0.2}


def roof_stain(x, y):
    """오래된 기와의 얼룩 — 저주파 큰 얼룩 + 중주파 + 아주 옅은 픽셀 결.
    합 진폭 ±1.5단 ≈ 명암 ±6. 딱 죽은 평면만 면할 만큼 (지시: ±6).
    셀 크기 8·4·1 은 16의 약수라 랩에서 끊기지 않는다."""
    return (smooth_octave(x, y, 8, 0x11, 0.72)
            + smooth_octave(x, y, 4, 0x23, 0.48)
...
def roof_rows(cracked=False):
    ...
    for y in range(16):
        row = []
        for x in range(16):
            v = ROOF_MID + ROOF_SWELL[x % 8] + ROOF_COURSE.get(y, 0.0) + roof_stain(x, y)
