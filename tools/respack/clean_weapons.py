...
def _sword_spec():
    """검 신병 — 곧고 가는 롱소드 + **정교한 금 십자 코등이** + 남색 자루/금 물미.
    ★25b 디테일 정교화 (레퍼런스 vid_zoom1 모사 · 앙상함 탈출): 코등이 = 상·하 **곡선 팔**
    (±22.5° 회전 큐보이드가 칼끝으로 휜다) + 양끝 **뾰족 finial** + 중앙 **파란 보석**(앞뒤 돌출·
    밝은 심 facet) + **층진 금 테**(어두운 몸 + 밝은 앞 하이라이트). 검신 = 청록 몸 + **fuller 홈**
    (능선 양옆 어두운 골) + 흰-청록 능선(볼록) — broad face 에 색층이 산다 (몸mid·홈dim·능선bright).
    색과 디테일이 같은 자리에서 만난다 (음영이 층을 읽어낸다)."""
    UP, LO = ("z", -22.5), ("z", 22.5)         # 상 팔 / 하 팔 회전축·각 (칼끝으로 휨)
    o_up, o_lo = (1.5, 9.4, 8.0), (1.5, 6.6, 8.0)   # 회전 원점 (팔이 뿌리에서 휜다)
    return [
        ...
        ((3.0, 7.45, 7.6, 17.0, 7.62, 8.4), "blade", ("z", 0, (0, 0, 0))),  # 아래 fuller 골 (dim 결)
        ((3.0, 8.38, 7.6, 17.0, 8.55, 8.4), "blade", ("z", 0, (0, 0, 0))),  # 위 fuller 골
        ...
        # ── 코등이: 정교한 금 십자 — 중앙 hub + 곡선 팔(회전) + finial + 층진 금 + 목 collar ──
        ((0.4, 6.6, 7.15, 2.6, 9.4, 8.85), "gold"),      # 중앙 hub (십자 몸 · 두껍게)
        ((0.7, 7.0, 8.6, 2.3, 9.0, 8.95), "gold"),       # hub 앞 하이라이트 층 (금속 광택)
        # 위 곡선 팔 (뿌리 9.4 → 칼끝 쪽으로 휜다) + 앞 하이라이트 + finial
        ((0.9, 9.4, 7.35, 2.1, 12.4, 8.65), "gold", (*UP, o_up)),      # 위 팔 몸
        ((1.1, 9.6, 8.55, 1.9, 12.4, 8.85), "gold", (*UP, o_up)),      # 위 팔 앞 하이라이트
        ((0.95, 12.4, 7.55, 2.05, 13.5, 8.45), "gold", (*UP, o_up)),   # 위 finial (뾰족)
        # 아래 곡선 팔 (대칭)
        ((0.9, 5.6, 7.35, 2.1, 8.6, 8.65), "gold", (*LO, o_lo)),       # 아래 팔 몸
        ((1.1, 5.6, 8.55, 1.9, 8.4, 8.85), "gold", (*LO, o_lo)),       # 아래 팔 앞 하이라이트
        ((0.95, 4.5, 7.55, 2.05, 5.6, 8.45), "gold", (*LO, o_lo)),     # 아래 finial (뾰족)
        ((2.5, 6.95, 7.45, 4.2, 9.05, 8.55), "gold"),    # 목 collar (langet — 날 위로 흐른 금)
        ((2.6, 7.35, 8.5, 3.9, 8.65, 8.7), "gold"),      # collar 앞 하이라이트
        # 중앙 파란 보석 — 앞뒤 돌출 + 밝은 심 facet
        ((0.7, 7.1, 8.6, 2.3, 8.9, 9.25), "gem"),        # 보석 앞 몸(+Z)
        ((1.15, 7.5, 9.15, 1.85, 8.5, 9.55), "gem"),     # 보석 앞 심 facet (돌출 · up=밝음)
...
def _dao_spec():
    ...
    spec += [
        ...
        ((0.75, 5.2, 8.5, 1.15, 10.8, 8.72), "gold", ("z", 0, (0, 0, 0))),  # 원반 앞 테 하이라이트 (림)
