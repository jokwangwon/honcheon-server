...
MUTATIONS = [
    ...
    # ─── ★조립 나룻배 (실기동 2026-07-25 — "마크 보트라 … 뱃사공도 없고") ───
    ("(104) 좌석을 안 가린다 (마크 보트가 겹쳐 보인다)", VOY,
     "        boat.setInvisible(!bargeParts.isEmpty());",
     "        boat.setInvisible(false);", "투명하지 않다"),
    ("(105) 나룻배를 등록부에서 뗀다 (맨몸 보트)", CFG,
     "      parts:                     # at = [앞x(동), 위y, 옆z] — 보트(좌석) 중심 기준",
     "      parts_없앰:                # at = [앞x(동), 위y, 옆z] — 보트(좌석) 중심 기준",
     "등록부에 없다"),
