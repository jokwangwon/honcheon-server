...
def build() -> dict:
    ...
    # ★ 아치를 **원점 중심으로** 옮긴다 (2026-07-21 · 사용자 ④: 궤적 중심이 몸 가운데가 아니다).
    #   호는 곡률중심(원점)에서 +Z 로만 뻗어 z 7~26 에 쏠려 있었다 — tracker 자리(z=0)가 아치 뒤라
    #   회전시키면 아치가 옆으로 돌았다. 무게중심을 원점에 맞추면 몸 한가운데에 걸린다.
    z_mid = (math.cos(math.radians(SWEEP) / 2.0) + 1.0) / 2.0 * RADIUS   # 아치 z 무게중심
    for i in range(SEGMENTS):
        ...
        cz = math.cos(th) * RADIUS - z_mid   # 무게중심을 원점으로 (몸 가운데)
