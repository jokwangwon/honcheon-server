"""V2-W 21차-d — 검을 휘감는 3D 번개(뇌전) 껍질. 스크래치 몽타주 (팩 무접촉·모듈 복원).
  v21d_lightning_wrap.png — 검 신병(옥빛 뇌)·도 마병(검붉은 뇌전) ×
...
def montage():
    ...
    d = ImageDraw.Draw(cv)
    d.text((22, 14), "V2-W 21차-d — 검을 휘감는 3D 번개(뇌전) 껍질  (기 포인트 → 뇌전 볼트로 대체)",
           font=f_t, fill=(236, 232, 245))
    d.text((22, 42), "번개가 날 표면에 흐르지 않고 검 외부에서 기를 대신해 날을 휘감는다 · float 볼트 "
                     "케이지(축 둘레 helix·다중 z·코플래너 아님) · frametime 3틱(감속) · 지그재그 척추+분기·흰-핫 코어",
    ...
    for lab, rots, frames in data:
        ...
        y += ch
    cv.convert("RGB").save(os.path.join(OUT, "v21d_lightning_wrap.png"))
    print("wrote v21d_lightning_wrap.png", cv.size)
